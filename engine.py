...
# Глобальный словарь оставшегося жилья (в масштабе агентов)
# {district: remaining_scaled_dwellings}
# Инициализируется из environment.json → vacant_dwellings × (n_agents / total_population)
# При переезде агента: вычитается AGENT_HOUSING_FOOTPRINT из целевого района,
# прибавляется к исходному.
HOUSING_REMAINING: dict[str, float] = {}

AGENT_HOUSING_FOOTPRINT = 1.1  # один агент занимает ~1.1 условных квартир
# (не ровно 1.0 — моделирует эффект «агент + члены семьи/сожители»)
...
def _get_effective_housing_price(
    housing_price_m2: float,
    district: str,
) -> float:
    """
    Вычисляет эффективную цену жилья с учётом динамического остатка квартир.

    delta = price_m2 × (AGENT_HOUSING_FOOTPRINT / remaining) × market_sensitivity
    effective_price = price_m2 + delta

    Чем меньше оставшегося жилья → тем выше дельта (конкуренция за жильё).
    Пол ограничен на уровне 1.0 — когда remaining ≤ 1 (меньше одного агента),
    цена фиксируется на максимальном уровне:
      delta_max = price_m2 × AGENT_HOUSING_FOOTPRINT × sensitivity
    Для BA (sens=1.75): effective_max ≈ price_m2 × 2.93
    Для стандартных (sens=1.0): effective_max ≈ price_m2 × 2.10
    """
    global HOUSING_REMAINING
    remaining = HOUSING_REMAINING.get(district, 1.0)

    # Получаем чувствительность рынка из кэша (заполняется в _init_housing_tracker)
    global _HOUSING_SENSITIVITY_CACHE
    sensitivity = _HOUSING_SENSITIVITY_CACHE.get(district, 1.0)

    # Пол = 1.0: при remaining ≤ 1 дельта максимальна, но не уходит в бесконечность
    delta = housing_price_m2 * (AGENT_HOUSING_FOOTPRINT / max(remaining, 1.0)) * sensitivity
    return housing_price_m2 + delta


# Кэш чувствительности рынка жилья (заполняется при инициализации из G.nodes)
_HOUSING_SENSITIVITY_CACHE: dict[str, float] = {}


def _init_housing_tracker(
    G: "nx.DiGraph",
    n_agents: int,
) -> None:
    """
    Инициализирует трекер оставшегося жилья HOUSING_REMAINING.

    Для каждого района:
      scaled_vacant = vacant_dwellings × (n_agents / total_population)
    где n_agents / total_population — масштабный коэффициент симуляции.

    Также заполняет _HOUSING_SENSITIVITY_CACHE из атрибутов узлов графа.
    """
    global HOUSING_REMAINING, _HOUSING_SENSITIVITY_CACHE
    ...
    for district in G.nodes:
        ...
        HOUSING_REMAINING[district] = max(1.0, vacant * scale)

        sensitivity = attr.get("housing_market_sensitivity", 1.0)
        _HOUSING_SENSITIVITY_CACHE[district] = float(sensitivity)

    # Диагностика
    total_scaled = sum(HOUSING_REMAINING.values())
    print(f"  HOUSING_REMAINING: {len(HOUSING_REMAINING)} районов")
    print(f"    total_scaled_vacant={total_scaled:,.1f}  "
          f"scale={scale:.6f}  n_agents={n_agents:,}")
    # Пример для Bratislava I
    ba1 = "District of Bratislava I"
    if ba1 in HOUSING_REMAINING:
        print(f"    {ba1}: remaining={HOUSING_REMAINING[ba1]:.1f}  "
              f"sensitivity={_HOUSING_SENSITIVITY_CACHE.get(ba1, 1.0)}")
...
def _housing_affordable(
    ...
    v11: Учитывает динамическую дельту цены из HOUSING_REMAINING.
    Если district не указан — используется номинальная цена (без дельты).
    """
    if agent_wage <= 0:
        return False

    if district and district in HOUSING_REMAINING:
        effective_price = _get_effective_housing_price(housing_price_m2, district)
    else:
...
def _execute_move(
    ...
    if old_residence and old_residence != new_residence:
        HOUSING_REMAINING[old_residence] = HOUSING_REMAINING.get(old_residence, 0.0) + AGENT_HOUSING_FOOTPRINT
    # Занимаем жильё в новом районе
    HOUSING_REMAINING[new_residence] = max(0.0, HOUSING_REMAINING.get(new_residence, 0.0) - AGENT_HOUSING_FOOTPRINT)
...
def _form_residence_candidates(
    ...
    for dst in candidates_raw:
        ...
        if not _housing_affordable(agent_wage, housing_price, district=dst):
            continue
...
def _unified_heuristic_search(
    ...
    if activation_dom == "economic":
        ...
        for dst_work in work_candidates:
            ...
            if _housing_affordable(agent_wage, housing_dst, district=dst_work):
                desired_raise = (target_wage / max(agent_wage, 1.0)) - 1.0 if agent_wage > 0 else 0.0
            ...
            for sat in satellites:
                ...
                if not _housing_affordable(agent_wage, sat_housing, district=sat):
                    continue
...
def _execute_housing_shock(
    ...
    """
    Применяет HOUSING_SHOCK к динамическому трекеру жилья.
    ...
    """
    global HOUSING_REMAINING
    if district not in HOUSING_REMAINING:
        return

    current = HOUSING_REMAINING[district]
    delta = current * magnitude  # доля от текущего остатка
    HOUSING_REMAINING[district] = max(0.0, current - delta)
    ...
    if abs(delta) > 0.5:
        ...
        print(f"  [tick {tick_num}] HOUSING_SHOCK {district}: "
              ...
              f"(magnitude={magnitude:.2f}, остаток={HOUSING_REMAINING[district]:.1f})")
...
def run_simulation(
    ...
    # v11: Инициализируем динамический трекер жилья
    _init_housing_tracker(G, len(df))
