...
def _get_effective_housing_price(
    ...
    Если remaining → 0, дельта растёт неограниченно → жильё недоступно.
    """
    global HOUSING_REMAINING
    remaining = HOUSING_REMAINING.get(district, 1.0)
    if remaining <= 0.01:
        # Жильё закончилось — огромная наценка
        return housing_price_m2 * 100.0

    # Получаем чувствительность рынка из графа (заполняется при инициализации)
    # Если граф недоступен, fallback = 1.0
    sensitivity = 1.0
    # Попытка получить из глобального кэша (заполняется в _init_housing_tracker)
    global _HOUSING_SENSITIVITY_CACHE
    sensitivity = _HOUSING_SENSITIVITY_CACHE.get(district, 1.0)

    delta = housing_price_m2 * (AGENT_HOUSING_FOOTPRINT / max(remaining, 0.01)) * sensitivity
